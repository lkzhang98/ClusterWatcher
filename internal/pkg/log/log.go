...
func C(ctx context.Context) *KLogger {
	return std.C(ctx)
}

func (k *KLogger) C(ctx context.Context) *KLogger {
	log := k.clone()

	if requestID := ctx.Value(known.XRequestIDKey); requestID != nil {
		log.z = log.z.With(zap.Any(known.XRequestIDKey, requestID))
	}

	if userID := ctx.Value(known.XUsernameKey); userID != nil {
		log.z = log.z.With(zap.Any(known.XUsernameKey, userID))
	}
